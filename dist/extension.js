...
function mergeSnapshots(existing, update) {
    if (!existing)
        return update;
...
async function refreshFooter(ctx, force = false) {
...
function parseCommandArgs(args) {
    ...
    for (const part of parts) {
        if (part === "json" || part === "raw" || part === "statusline" || part === "status")
            kind = part;
        else if (part === "refresh" || part === "--refresh" || part === "-r")
            refresh = true;
        else
            throw new Error(`Unknown /status argument: ${part}`);
    }
    return { kind, refresh };
}
