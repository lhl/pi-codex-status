...
export function normalizeWindow(window, nowMs = Date.now()) {
    ...
    const resetAfterSeconds = numberValue(obj.reset_after_seconds) ??
...
function normalizeRateLimit(id, name, apiRateLimit, options = {}) {
    if (!apiRateLimit)
        return undefined;
...
export function normalizeApiUsage(api, meta = {}) {
...
export async function getCodexUsage(options = {}) {
    ...
    if (!options.noCache) {
        ...
        if (isFresh(cached, maxAgeMs))
            return { ...cached, source: cached.source === "cache" ? "cache" : cached.source };
    }
