...
export function extractAccountClaims(...tokens) {
    const result = {};
    for (const token of tokens) {
        ...
        if (result.accountId === undefined && accountId !== undefined)
            result.accountId = accountId;
        const plan = stringValue(auth?.chatgpt_plan_type);
        if (result.plan === undefined && plan !== undefined)
            result.plan = plan;
        const email = stringValue(profile?.email) ?? stringValue(payload.email);
        if (result.email === undefined && email !== undefined)
            result.email = email;
